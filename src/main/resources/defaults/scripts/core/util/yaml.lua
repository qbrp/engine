...
table_print_value = function (value, indent, done)
    ...
    if type(value) == "table" and not done[value] then
        ...
        for _, key in ipairs(list) do
            ...
            rep = rep
                .. string.format(
                    "%s[%s] = %s%s\n", string.rep(" ", indent + 2), keyRep,
                    table_print_value(value[key], indent + 2, done), comma
                )
        end
...
-- Lua tables do not preserve the source order of string keys for iteration.
-- Keep YAML mapping order out-of-band so parsed documents remain ordinary tables.
local mapping_orders = setmetatable({}, { __mode = "k" })
local mapping_keys = setmetatable({}, { __mode = "k" })

local function mapping_put(mapping, key, value)
    local order = mapping_orders[mapping]
    local keys = mapping_keys[mapping]
    if not order then
        order = {}
        keys = {}
        mapping_orders[mapping] = order
...
---Iterates a parsed YAML mapping in source order.
---Falls back to ordinary table iteration for tables not created by this parser.
---@param mapping table
---@return function
function exports.mapping_pairs(mapping)
    local order = mapping_orders[mapping]
    if not order then
...
local tokens = {
    { "comment", "^#[^\n]*" }, { "indent", "^\n( *)" }, { "space", "^ +" },
    { "true", word("enabled"), const = true, value = true }, { "true", word("true"), const = true, value = true },
    { "true", word("yes"), const = true, value = true }, { "true", word("on"), const = true, value = true },
    { "false", word("disabled"), const = true, value = false }, { "false", word("false"), const = true, value = false },
    { "false", word("no"), const = true, value = false }, { "false", word("off"), const = true, value = false },
    { "null", word("null"), const = true, value = nil }, { "null", word("Null"), const = true, value = nil },
    { "null", word("NULL"), const = true, value = nil }, { "null", word("~"), const = true, value = nil },
    { "id", "^\"([^\"]-)\" *(:[%s%c])" }, { "id", "^'([^']-)' *(:[%s%c])" },
    { "string", "^\"([^\"]-)\"", force_text = true }, { "string", "^'([^']-)'", force_text = true },
...
Parser.parse = function (self)
    ...
    local result
    local c = { indent = self:accept("indent") and 1 or 0, token = self:peek() }
...
Parser.parseHash = function (self, hash)
    ...
    if self:isInline() then
        ...
        if self:accept("indent") then
            indents = indents + 1
            mapping_put(hash, id, self:parse())
        else
            mapping_put(hash, id, self:parse())
            if self:accept("indent") then
                indents = indents + 1
